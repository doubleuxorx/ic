...
const AppInner = () => {
  ...
  }, [workspace, canvasPath, document, selectionIds, selectedEdgeIds, activeNodeId]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const fromArguments = useWorkspaceStore.getState().facts?.initialWorkspace ?? null;
      ...
      if (remembered && isDesktop()) {
        try {
          ...
          if (last) await useCanvasStore.getState().load(last).catch(() => undefined);
        } catch {
    ...
    void (async () => {
      ...
      for (const record of records) {
        ...
        if (restore) {
          useDocumentStore
            .getState()
            .adoptRecovery(record.relativePath, record.contents, record.baseRevision);
        } else {
  ...
  useEffect(() => {
    ...
    const unlisten = listen<{ paths?: string[] }>('tauri://drag-drop', (event) => {
      ...
      void (async () => {
        for (const path of paths) {
          try {
            const facts = await ipc.attachmentImport(path, DEFAULT_DIRECTORIES.attachments);
            if (useCanvasStore.getState().path) addFileNode(facts.relativePath);
  ...
  return (
    <div className="app">
      ...
      <div className="status-bar">
        ...
        {saving ? <span>saving</span> : dirty ? <span className="dirty">unsaved</span> : null}
        {viEnabled ? <span>vi</span> : null}
        {selectionIds.length > 0 ? <span>{selectionIds.length} selected</span> : null}
      </div>
      ...
      {conflict ? (
        <div className="toast error">
          ...
          <button
            ...
            onClick={() => void useCanvasStore.getState().resolveConflict('take-disk')}
          >
            Use disk version
          </button>
          <button
            ...
            className="icon-button"
            style={{ width: 'auto', padding: '0 8px' }}
            onClick={() => void useCanvasStore.getState().resolveConflict('keep-mine')}
          >
            Keep mine
          </button>
        </div>
