...
export const canActivateFileKind = (
	kind: FileKind | null | undefined,
): boolean => kind === "markdown" || kind === "text" || kind === "pdf";
...
export const useFileFacts = (relativePath: string): FactsState => {
	...
	useEffect(() => {
		...
		setState((previous) => ({
			facts:
				previous.facts?.relativePath === relativePath ? previous.facts : null,
			error: null,
			loading: true,
		}));
		ipc
			...
			.catch((error) => {
				if (!cancelled)
					setState({ facts: null, error: errorMessage(error), loading: false });
			});
		return () => {
			cancelled = true;
		};
