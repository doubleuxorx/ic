...
Video and audio reach the webview differently on Linux. WebKitGTK decodes
through GStreamer, which fetches only the schemes it knows, so a file served
from `ic://` never reaches a decoder and the element reports an unsupported
source however ordinary the file is. There, and only there, the same files are
served over loopback HTTP by `media::server` — still range by range, so seeking
does not buffer the whole file. Its rules are in the security model. A source a
webview refuses for any other reason still leads to the offer of the system
player.
